...
class Bot_Sniper(BaseAgent):
    ...
    def act(self, state: PlayerState) -> Action:
        ...
        # BARDZO wysokie wymagania (gra tylko "monsterami")
        min_strength = 0.65 if len(state.community_cards) == 0 else 0.80

        if strength > min_strength:
            # Ma potwora -> Przebija mocno
            return Action(ActionType.RAISE, amount=state.min_raise * 2)
        
        # Jeśli check jest za darmo, to sprawdza
