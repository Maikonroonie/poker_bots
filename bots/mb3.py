...
class Bot_Bully(BaseAgent):
    ...
    def act(self, state: PlayerState) -> Action:
        ...
        # Niskie wymagania wejścia (gra luźno)
        is_preflop = len(state.community_cards) == 0
        threshold = 0.35 if is_preflop else 0.50

        # Jeśli ręka jest OK, prawie zawsze przebija
        if strength > threshold:
            if random.random() < 0.8: 
                # Overbet (duże przebicie)
                bet = int(state.pot * 0.75) + state.min_raise
                return Action(ActionType.RAISE, amount=bet)
            return Action(ActionType.CHECK_CALL)

        # BLEF: Czasem przebija nawet ze śmieciami (10% szans)
        if state.current_bet > 0 and random.random() < 0.1:
